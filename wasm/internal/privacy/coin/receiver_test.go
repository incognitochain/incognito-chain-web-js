...
func TestOTAReceiverFromAddressWithoutConceal(t *testing.T) {
    type Testcase struct {
        name     string
        data     string
    }
    ...
    testcases := []Testcase{
        Testcase{"Valid", "12sxXUjkMJZHz6diDB6yYnSjyYcDYiT5QygUYFsUbGUqK8PH8uhxf4LePiAE8UYoDcNkHAdJJtT1J6T8hcvpZoWLHAp8g6h1BQEfp4h5LQgEPuhMpnVMquvr1xXZZueLhTNCXc8fkebLV8nDoJ17"},
        Testcase{"Valid 2", "12si2KgWLGuhXACeqHGquGpyQy7JZiA5qRTCWW7YTYrEzZBuZC2eGBfckc2NRXkQXiw7XwK2WVfKxC8AcwKGCsyRVr9SR8bN9vTcnk2PPbymztCWadgr9JMP1UY6oSk9XZb56EAKvK1fJd5S8ptY"},
    }
    ...
    for _, testcase := range testcases {
        t.Run(testcase.name, func(t *testing.T) {
            ...
            p = p.ScalarMult(p, sc)
            True(t, p.IsIdentity())
            // fmt.Printf("conceal point %s", p.String())
        })
    }
}
