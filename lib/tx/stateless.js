...
import bridgeTxs from '@lib/tx/bridge.tx';
import pdexv3Txs from '@lib/tx/pdexv3.tx';
...
class StatelessTransactor extends Account {
     ...
     */
    async init(rpcClient) {
        let rpc = new RpcClient(rpcClient);
        this.setRPCClient(rpcClient);

        // duct-tape methods to skip services
        this.getUnspentCoinsV2 = async function({ tokenID, version }) { 
            if (!this.hwTransport) return this.getListUnspentCoins(tokenID, 2);
            const res = await this.scanCoins({ tokenList: [tokenID] });
            if (tokenID === PRVIDSTR) return res[PRVIDSTR].unspentCoins;
            else return res[PTOKEN_ID].unspentCoins.filter(c => c.TokenID === tokenID);
        }
        ...
        this.rpcTxService = {
            ...
            apiPushTx: async function({ rawTx }) {
                try {
                    ...
                    if (e.Code == -1003 && typeof(e.StackTrace) == 'string') {
    ...
    async getBalance({tokenID = PRVIDSTR, version = 2} = {}) {
        try {
            const listUnspentCoins = await (version == 1 ?  this.getListUnspentCoins(tokenID, 1) : this.getUnspentCoinsV2({ tokenID, version }));
...
let trimHexPrefix = (s) => s.startsWith('0x') ? s.slice(2) : s;
...
class TxBuilder {
    ...
    async send() {
        ...
        this.result = await this.transactor.transact(this.params).catch(console.error);
        return this;
    }
    ...
    useHw(enabled, firstC) {
        this.params.extra.useHw = Boolean(enabled);
        if (enabled) {
            this.params.extra.firstC = firstC;
        }
        return this;
    }
...
Object.assign(TxBuilder.prototype, pdexv3Txs, bridgeTxs);
