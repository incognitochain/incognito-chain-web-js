...
let PrivacyGoMethods = null;

const setPrivacyGoMethods = (_module) => {
    PrivacyGoMethods = _module;
}

const wasm = new Proxy(methods, {
    get(_, key) {
        if (PrivacyGoMethods) {
            if (!(key in PrivacyGoMethods)) {
                return () => Promise.reject(`privacy-go-methods: name "${key.toString()}" not found`);
            }
            return PrivacyGoMethods[key];
        }
