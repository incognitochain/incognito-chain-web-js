...
import { getAddress, getViewKey, getOTAKey, TransportHttp } from 'hw-app-incognito';
...
class Wallet {
  ...
  async initAccountWithHardwareWallet(accountName = "__hw", account, emulator = false, url = 'http://127.0.0.1:9998') {
    if (!account) account = new Account(Wallet);
    try {
      for (const acc of this.MasterAccount.child) {
        if (Boolean(acc.hwTransport)) {
      ...
      const addr = await getAddress(t); // get viewkey, otakey, address
      // const vk = await getViewKey(t);
      ...
      for (let i = 0; i < this.MasterAccount.child.length; i++) {
        if (this.MasterAccount.child.name == accountName) {
          this.MasterAccount.child[i] = account;
          exists = true;
          break;
        }
      }
      if (!exists) this.MasterAccount.child.push(account);
      return account;
    } catch(e) {
  ...
  async save(password = "", legacyEncryption = false) {
    try {
      ...
      wallet.MasterAccount.child = wallet.MasterAccount.child.filter(acc => acc.key.KeySet.PrivateKey?.length === 32).map((account) =>
    ...
    try {
      ...
      if (this.Storage) {
        ...
        try {
          ...
          obj.MasterAccount.child = obj.MasterAccount.child.filter(acc => acc.key.KeySet.PrivateKey?.length === 32); // filter out hw accounts
          let task = [
