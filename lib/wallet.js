import _, { cloneDeep, isEqual, lowerCase, set, toLower, toString, uniqBy, includes } from "lodash";
...
import { CustomTokenTransfer, CustomTokenInit, MAX_INPUT_PER_TX } from "./tx/constants";
...
import { ENCODE_VERSION, ED25519_KEY_SIZE, setShardNumber as setShardNumberSub } from "./common/constants";
...
import { getAddress, switchKey, getOTAKey, TransportHttp } from "hw-app-incognito";
...
const MATER_KEYS_BACKUP_BY_PREFIX_SERVER_ID = "MATER_KEYS_BACKUP_BY_PREFIX_SERVER_ID";
...
class Wallet {
  ...
  async measureImport(mnemonic, passPhrase, name, storage) {
    try {
      ...
      await this.measureAsyncFn(() => this._generateMasterAccount(), "importWallet.generateMasterAccount");
      await this.measureAsyncFn(() => this._generateFirstAccount(), "importWallet.generateFirstAccount");
    } catch (error) {
  ...
  getAccountIndexByName(accountName) {
    try {
      new Validator("getAccountIndexByName-accountName", accountName).required().string();
      const index = this.MasterAccount.child.findIndex((item) => isEqual(toLower(item.name), toLower(accountName)));
      return index;
    } catch (error) {
      throw error;
    }
  ...
  async measureCreateNewAccount(accountName, shardID, excludedIds) {
    try {
      new Validator("createNewAccount-accountName", accountName).required().string();
  ...
  async measureCreateAccount(accountName, shardID, excludedIds) {
    try {
      new Validator("createAccount-accountName", accountName).required().string();
      ...
      while (true) {
        ...
        const isExisted = createdPrivateKeys.includes(newPrivateKey) || createdIds.includes(newId);
        if (
          isExisted ||
          (_.isNumber(shardID) && getShardIDFromLastByte(lastByte) !== shardID) ||
  ...
  async switchKeyHardwareWallet(index = ACCOUNT_CONSTANT.DEFAULT_HARDWARE_ACCOUNT_INDEX) {
    const temp = this.MasterAccount.child.filter((acc) => acc.hwIndex > 0);
    if (temp.length != 1) throw `invalid number of hardware accounts ${temp.length}`;
    const hwacc = temp[0];
    // if (!hwacc.hwTransport) throw 'invalid hardware account';
    const { PaymentAddressBytes, IndexBytes } = await switchKey(hwacc.hwTransport, index);
    hwacc.hwIndex = index;
    try {
      let t = hwacc.hwTransport
        ? hwacc.hwTransport
        : await (emulator ? TransportHttp([url]) : TransportWebHID).create();
      // get address, otakey from device
      let keyWallet = new KeyWallet();
      const otakey = await getOTAKey(t);
      const addr = await getAddress(t); // get otakey, address
      // create KeyWallet object
      await keyWallet.KeySet.importWithoutPrivateKey(addr, null, otakey);
      hwacc.key = keyWallet;
      await hwacc.setStorageHardwareAccountIndex(hwacc.hwIndex);
      return hwacc;
    } catch (e) {
      console.log("error switch account hardware", e);
      throw e;
    }
  }

  async initAccountWithHardwareWallet(accountName = "__hw", account, emulator = false, url = "http://127.0.0.1:9998") {
    if (!account) {
      // init new account, unless wallet already contains hardware device account
      for (const acc of this.MasterAccount.child) {
        if (acc.hwIndex > 0) {
          console.log("use existing hardware wallet account");
          return acc;
        }
      }
    ...
    try {
      ...
      account.isImport = true;
      account.name = accountName;
      account.hwTransport = t;
      // save current hardware index
      if (!account.hwIndex) {
        const indexFromStorage = await account.getStorageHardwareAccountIndex();
        if (!indexFromStorage) account.hwIndex = ACCOUNT_CONSTANT.DEFAULT_HARDWARE_ACCOUNT_INDEX;
      }
      await account.setStorageHardwareAccountIndex(account.hwIndex);

      this.MasterAccount.child.push(account);
      return account;
    } catch (e) {
      console.log("error init account hardware", e);
      throw e;
    }
  }
  ...
  validatePrivateKey(privateKey) {
    new Validator("validatePrivateKey-privateKey", privateKey).required().string();
    let existed = false;
    for (let i = 0; i < this.MasterAccount.child.length; i++) {
      let account = this.MasterAccount.child[i];
  ...
  async removeAccount(accPrivateKeyStr) {
    let removed = false;
    try {
      new Validator("removeAccount-accPrivateKeyStr", accPrivateKeyStr).string().required();
      const temp = await this.createAccountWithPrivateKey(accPrivateKeyStr, "temp");
  ...
  async importAccount(privakeyStr, accountName, passPhrase) {
    try {
      ...
      new Validator("importAccount-privakeyStr", privakeyStr).string().required();
      new Validator("importAccount-accountName", accountName).string().required();
      new Validator("importAccount-passPhrase", passPhrase).string();
      let account = await this.createAccountWithPrivateKey(privakeyStr, accountName);
  ...
  async measureImportAccountWithId(accountId, accountName, index) {
    try {
      ...
      new Validator("importAccountWithId-accountName", accountName).string().required();
      try {
        this.validateAccountName(accountName);
      } catch {
        this.measureImportAccountWithId(accountId, `${accountName}-${index}`, index + 1);
      }
  ...
  async save(password = "", legacyEncryption = false) {
    try {
      ...
      wallet.MasterAccount.child = wallet.MasterAccount.child
        .filter((acc) => !acc.hwIndex)
        .map((account) => this.getAccountWillBeStoraged(account));
  ...
  async reImportPrototype(account) {
    try {
      ...
      await newAccount.key.KeySet.importFromPrivateKey(newAccount.key.KeySet.PrivateKey);
      return newAccount;
    } catch (error) {
  ...
  async measureLoadWallet(passphrase) {
    try {
      ...
      if (this.Storage) {
        ...
        if (!cipherText) {
          await this.setStorageLoadWalletError({
            error: {
              name: selfRootname,
        ...
        if (!newMethodDecrypted) {
          try {
            ...
            throw new CustomError(ErrorObject.LoadWalletErr, "Error when load wallet by CryptoJS", error);
          }
        }
        try {
          ...
          obj.MasterAccount.child = obj.MasterAccount.child.filter((acc) => !acc.hwIndex); // filter out hw accounts
          let task = [...obj.MasterAccount.child.map((account) => this.reImportPrototype(account))];
          ...
          obj.MasterAccount.child = cloneDeep(
            uniqBy(obj.MasterAccount.child, (acc) => [acc.name, acc.getPrivateKey()].join())
          );
          ...
          throw new CustomError(ErrorObject.LoadWalletErr, "Error when load wallet", error);
        }
      }
    } catch (error) {
      throw error;
  ...
  async loadWallet(passphrase) {
    try {
      const wallet = await this.measureAsyncFn(() => this.measureLoadWallet(passphrase), "loadWallet.totalTime");
      return wallet;
    } catch (error) {
      throw error;
    }
  ...
  async listAccount() {
    try {
      let task = this.MasterAccount.child.map((account) => account.getDeserializeInformation());
  ...
  async listAccountNoCache() {
    try {
      let task = this.MasterAccount.child.map((account) => account.getDeserializeInformationNoCache());
      let result = await Promise.all(task);
      result = result.map((info, index) => ({ ...info, Index: index }));
      return result;
    } catch (error) {
      console.log("listAccountNoCache error", error);
      throw error;
    }
  }
  ...
  async setKeyMeasureStorage() {
    try {
      ...
      let value = [];
      value = oldMeasureStorage.length > 0 ? [this.measureStorage, ...oldMeasureStorage] : [this.measureStorage];
      await this.setWalletStorage({
        key,
        value,
      });
  ...
  async setWalletStorage(params) {
    ...
    try {
      ...
      if (typeof this.Storage.setItem === "function" && value && key) {
        await this.Storage.setItem(key, typeof value !== "string" ? JSON.stringify(value) : value);
      }
    } catch (error) {
  ...
  async setStorageBackupMasterKey(wallet, passphrase) {
    try {
      const { aesKey } = passphrase;
      new Validator("setStorageBackupMasterKey-aesKey", aesKey).string().required();
      new Validator("setStorageBackupMasterKey-Wallet", wallet).object().required();
      const key = this.getKeyStorageBackupMasterKey();
      let rootName = toLower(wallet?.RootName);
      wallet.IsMasterless = includes(rootName, "masterless") || includes(rootName, "unlinked");
      ...
      const foundIndex = oldList.findIndex((w) => isEqual(_wallet?.mnemonic, w?.mnemonic));
      ...
      if (!isExisted && !isMasterless) {
        ...
        const combineAccounts = _.uniqBy([...oldListAccounts, ...newListAccounts], "privateKey");
        newList[foundIndex].accounts = combineAccounts;
      }
      const cipherText = sjcl.encrypt(sjcl.codec.hex.toBits(aesKey), JSON.stringify(newList));
