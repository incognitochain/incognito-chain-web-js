...
class Wallet {
  ...
  async initAccountWithHardwareWallet(accountName = "__hw", account, emulator = false) {
    if (!account) account = new Account(Wallet);
    try {
      let t = account.hwTransport? account.hwTransport : 
        await (emulator ? TransportHttp(['http://127.0.0.1:9998']).create()
    ...
    try {
      ...
      if (this.Storage) {
        ...
        try {
          ...
          obj.MasterAccount.child.push(await this.initAccountWithHardwareWallet("__hw", true));
