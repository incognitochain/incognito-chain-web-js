...
import { TransportHttp, getAddress, getOTAKey, switchKey } from 'hw-app-incognito';
...
import { hashSha3BytesToBytes, setRandBytesFunc } from "./privacy/utils";
import { hybridDecryption, hybridEncryption } from "./privacy/hybridEncryption";
import { isOldPaymentAddress, isPaymentAddress } from "./utils/paymentAddress";
...
class Wallet {
  ...
  async switchKeyHardwareWallet(index = ACCOUNT_CONSTANT.DEFAULT_HARDWARE_ACCOUNT_INDEX) {
    ...
    hwacc.hwIndex = index;
    try {
      let t = hwacc.hwTransport ? hwacc.hwTransport :
        await (emulator ? TransportHttp([url])
  ...
  async initAccountWithHardwareWallet(accountName = "__hw", account, emulator = false, url = 'http://127.0.0.1:9998') {
    ...
    try {
      let t = account.hwTransport ? account.hwTransport :
        await (emulator ? TransportHttp([url])
          : TransportWebHID).create();
      // get address, otakey from device
      let keyWallet = new KeyWallet();
  ...
  async save(password = "", legacyEncryption = false) {
    try {
      ...
      wallet.MasterAccount.child = wallet.MasterAccount.child.filter(acc => !acc.hwIndex).map((account) =>
  ...
  async listAccountNoCache() {
    try {
      let task = this.MasterAccount.child.map((account) =>
        account.getDeserializeInformationNoCache()
      );
      let result = await Promise.all(task);
      result = result.map((info, index) => ({ ...info, Index: index }));
      return result;
