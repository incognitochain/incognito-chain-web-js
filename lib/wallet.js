...
function parseStorageBackup({ passphrase, backupStr }) {
  ...
  try {
    ...
    new Validator("parseStorageBackup-aesKey", aesKeyDefault)
      .string()
      .required();
...
class Wallet {
  ...
  async hasCreatedAccount(privateKey) {
    ...
    const childId = getChildIdFromChildNumberArray(newAccount.key.ChildNumber);
  ...
  async createAccount(accountName, shardID, excludedIds = []) {
    try {
      ...
      const firstAccount = createdAccounts.find(
        (account) => account.ID === 1 || account.Index === 0
      );
      if (!!firstAccount) {
      ...
      if (this.Storage) {
        ...
        try {
          ...
          try {
            ...
            await this.setStorageBackupMasterKey(
              this,
              passphrase,
  ...
  async setStorageBackupMasterKey(
    wallet,
    passphrase,
