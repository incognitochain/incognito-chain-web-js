...
function removePrefixContract({ contract } = {}) {
  new Validator("removePrefixContract-contract", contract).required().string();
...
function getSwapStorageTxIDsByNetworkName({ callContract } = {}) {
  new Validator("getSwapStorageTxIDsByNetworkName-callContract", callContract).required().string();
...
async function getStorageSwapHistoryByCallContract({ callContract } = {}) {
  new Validator("getStorageSwapHistory-callContract", callContract).required().string();
  callContract = this.removePrefixContract({ contract: callContract });
...
async function setStorageSwapHistoryByCallContract({
  ...
  new Validator("setStorageSwapHistoryByCallContract-callContract", callContract)
    .required()
    .string();
  new Validator("setStorageSwapHistoryByCallContract-txHash", txHash).required().string();
  new Validator("setStorageSwapHistoryByCallContract-sellTokenID", sellTokenID).required().string();
  new Validator("setStorageSwapHistoryByCallContract-buyTokenID", buyTokenID).required().string();
  ...
  swapTxs = swapTxs.filter((tx) => tx.txHash !== txHash);
  swapTxs.push({
    txHash,
...
async function getSwapHistoryStorage({ callContracts } = {}) {
  let txs = [];
  new Validator("getSwapHistoryStorage-callContract", callContracts).required().array();
  ...
  try {
    ...
    txs = orderBy(flatten(txs) || [], "time", "desc").slice(0, NUMBER_TXS);
  } catch (e) {
    throw e;
  }

  console.log('getSwapHistoryStorage: txs: ', { txs })

  // swapTxID backend swap service
  const swapTxIDs = [];
  const swapInterTxIDs = [];
  for (const localTx of txs) {
    const isInter = !!localTx?.interPAppName;
    if (localTx.txHash) {
      if (isInter) {
  ...
  console.log('getSwapHistoryStorage: swapTxIDs: ', { swapTxIDs })
  console.log('getSwapHistoryStorage: swapInterTxIDs: ', { swapInterTxIDs })

  const [historySwapApi, historyInterApi] = await Promise.all([
    (await this.webAppService.getSwapTxStatus({
      txIDs: swapTxIDs,
    })) || {},
  (await this.webAppService.getInterSwapTxStatus({
      txIDs: swapInterTxIDs,
    })) || {},

  ])
  console.log('getSwapHistoryStorage: historyInterApi: ', { historyInterApi });
  console.log('getSwapHistoryStorage: historySwapApi: ', { historySwapApi });

  const history = this.combineSwapTxs({
    txsStorage: txs,
    txsApi: {
      ...historySwapApi,
      ...historyInterApi,
