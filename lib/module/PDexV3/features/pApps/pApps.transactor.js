import { bridgeaggMeta } from "@lib/core/constants";
...
async function createTransactionPApps(params) {
    try {
        ...
        // Render Redeposit OTA
        const otaRedepositReceiver = interSwapData ?
            interSwapData?.midOTA :
            await this.account.getOTAReceive();
        ...
        const statusPayload = {
            callContract,
            txHash: txId,
            sellTokenID: interSwapData?.sellTokenID || sellTokenID,
            buyTokenID: interSwapData?.buyTokenID || buyTokenID,
            sellAmountText,
            buyAmountText,
