...
import { checkWithdrawableContribute, groupAccessOTAShare } from "@lib/module/PDexV3/features/Share/share.utils";
...
async function getShareWithNFTID() {
  ...
  try {
    ...
    const tasks = nftIds.map(async(nftId) => {
      const res = (await this.rpcTradeService.apiGetListShare({ nftId })) || []
      return res.map((item) => {
        item = {
          ...camelCaseKeys(item),
          nftId,
...
async function getShareWithOTAKey() {
  ...
  try {
    ...
    // Get Burning Status
    const tasks = [
      (await this.rpcTradeService.apiGetListShare({ otaKey })) || [],
      await Promise.all(
  ...
  return groupAccessOTAShare(share);
}
