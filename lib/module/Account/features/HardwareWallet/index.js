import { calculateR, calculateFirstC, calculateCoinPrivKey, genAlpha, setAlpha, calculateKeyImage, decryptCoin, confirmTx } from 'hw-app-incognito';
...
async function hwBeforeCreate(hwTransport, paymentInfoLst, inputCoins, tokenInputCoins, debugAlphaLst = null, tokenDebugAlphaLst = null) {
    let result = []
    {
        if (!debugAlphaLst) {
    ...
    for (const pInfo of paymentInfoLst) {
        const w = KeyWallet.base58CheckDeserialize(pInfo.PaymentAddress);
        let keyBytes = new Uint8Array(96);
        keyBytes.set(w.KeySet.PaymentAddress.Pk, 0);
        keyBytes.set(w.KeySet.PaymentAddress.Tk, 32);
        keyBytes.set(w.KeySet.PaymentAddress.OTAPublic, 64);
        // display TX receiver in device
        await confirmTx(hwTransport, pInfo.Amount, 9, base64Encode(keyBytes)); // NOTE: when to use different decimals
    }
    return result;
}
