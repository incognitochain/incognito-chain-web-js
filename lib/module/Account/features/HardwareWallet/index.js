...
async function hwAfterCreate(hwTransport, txObj, inputCoins, inputCoinPosition, tokenInputCoins, tokenInputCoinPosition) {
    {
    ...
    if (tokenInputCoins?.length > 0) {
        // decode partial ring signature
        let sig = Buffer.from(txObj.TxTokenPrivacyData.Sig, 'base64');
        console.info('token sig phase 1', txObj.TxTokenPrivacyData.Sig);
        const sigObj = decodeSig(sig, true);
        console.info('decoded signature', sigObj);
        ...
        // fill out ring sig
        const rLst = await calculateR(hwTransport, sigObj.numInputs + 1, sigObj.cpi);
        console.info('coins row before', sigObj.R[tokenInputCoinPosition]);
        sigObj.R[tokenInputCoinPosition] = rLst;
        console.info('coins row after', sigObj.R[tokenInputCoinPosition]);
        txObj.tokenInputCoins.Sig = encodeSig(sigObj);
        console.info('token sig phase 2', txObj.tokenInputCoins.Sig);
    }
    return txObj;
}
