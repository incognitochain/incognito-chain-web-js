    ...
    for (let i = start; i < coinlen; i += batchSize * poolSize) {
      ...
      for (let taskInd = 0; taskInd < poolSize; taskInd++) {
        ...
        for (let j = 0; j < batchSize; j++) {
          if (currentCoinIndex < coinlen - 1) indexes.push(currentCoinIndex++);
          else break;
        }
        tasks.push(this.rpc.getOutputCoinByIndex(indexes, shardID, tokenID).then(async coinMap => {
          for (const k in Object.keys(coinMap)) {
            const c = coinMap[k];
            try {
              const coin = JSON.parse(await wasm.decryptCoin(JSON.stringify({
                Coin: c,
                KeySet: privkey
              })));
              c.Index = k;
              resultForToken.push(c);
            } catch (e) {
