import { OTAKeyType, PrivacyVersion } from "@lib/core/constants";
...
async function submitOTAKey({ version = PrivacyVersion.ver2 } = {}) {
  try {
    const otaKey = this.getOTAKey();
    ...
    if (!submitted) {
      ...
      switch (version) {
        case PrivacyVersion.ver1:
        case PrivacyVersion.ver2:
          result = await this.rpcCoinService.apiSubmitOTAKey({
            otaKey,
            shardID,
          });
          break;
