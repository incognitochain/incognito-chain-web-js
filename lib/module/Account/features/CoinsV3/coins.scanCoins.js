...
export const ScanCoinBatchSize = 1000, MaxBatchCount = 300, ScanCoinPoolSize = 5, ScanCoinSpawnWorkerThreshold = 5000; //, ScanCoinDecryptSize = 1000;
const rpcOutcoinSemaphore = new Semaphore(5);
const updateResultSemaphore = new Semaphore(1);
const updateStorageSemaphore = new Semaphore(1);
...
async function scanCoins(params) {
    ...
    try {
        ...
        if (!!coinsStore) {
            params = { ...params, ...coinsStore, tokenList };
            ...
            if (params[PTOKEN_ID] && params[PTOKEN_ID].unspentCoins && params[PTOKEN_ID].unspentCoins.length > 0) {
        ...
        coins = await this.activeScanCoinsWithWebWorkers(params);
    } catch (error) {
...
async function activeScanCoins(cfg = {}) {
    ...
    for (const tokenID of [PRVIDSTR, PTOKEN_ID]) {
        ...
        const lastIndex = result[tokenID].latestIndex || -1;
...
async function activeScanCoinsWithWebWorkers(cfg = {}) {
    const tokenList = cfg.tokenList || [];
    const shardID = getShardIDFromLastByte(this.key.KeySet.PaymentAddress.Pk[(this.key.KeySet.PaymentAddress.Pk.length - 1)]);
    const lengths = await this.rpc.getCoinCount();
    const privkey = (this.key.KeySet.PrivateKey?.length == 32) ? this.key.base58CheckSerialize(PriKeyType)
    ...
    let result = cloneDeep(cfg);
    let started = false;
    const tokenIDs = [PRVIDSTR, PTOKEN_ID];
    ...
    for (const tokenID of tokenIDs) {
        ...
        const lastIndex = result[tokenID].latestIndex || -1;
        const newLatestBatchID = Math.floor(coinlen / ScanCoinBatchSize);
        let batchesToScan = Object.keys(result[tokenID].missingBatchIDs || {}).map(k => Number(k));
        if (!result[tokenID].latestBatchID) result[tokenID].latestBatchID = 0;
        let i = result[tokenID].latestBatchID;
        if (!(result[tokenID].missingBatchIDs[i])) { // avoid duplicates
            batchesToScan.push(i);
        }
        for (i = result[tokenID].latestBatchID + 1; i <= newLatestBatchID; i++) {
            batchesToScan.push(i);
        }
        
        for (const bID of batchesToScan) result[tokenID].missingBatchIDs[bID] = true;

        console.log(
            'shard', shardID,
            'token', tokenID,
            'lastIndex', lastIndex,
            'list batchIDs to scan', batchesToScan
        );
        i = 0;
        tasks.push(batchesToScan.map(async bID => {
            try {
                let batchStart = bID * ScanCoinBatchSize;
                let batchEnd = batchStart + ScanCoinBatchSize - 1;
                ...
                console.log(`activeScanCoins: scan coins for batch #${bID}, token ${tokenID.slice(58)}`);
                const coinMap = await rpcOutcoinSemaphore.use(() => this.rpc.getOutputCoinByIndex(batchStart, batchEnd, shardID, tokenID));

                console.log(`activeScanCoins: finish request coins for batch #${bID}, token ${tokenID.slice(58)}`);
                let decryptedCoinList = JSON.parse(await wasm.decryptCoinList(JSON.stringify({
                    CoinList: Object.values(coinMap),
                    KeySet: privkey,
                    TokenList: tokenList,
                }))) || [];
                if (this.hwTransport) {
                    // handling for hardware device key
                    for (let c of decryptedCoinList) {
                        await decryptCoinAmount(this.hwTransport, c);
                        await parseCoinKeyImage(this.hwTransport, c);
                    }
                }
                delete result[tokenID].missingBatchIDs[bID];
                
                await updateResultSemaphore.use(async () => {
                    result[tokenID].unspentCoins = uniqBy(decryptedCoinList.concat(result[tokenID].unspentCoins || []), "KeyImage");
                    if (result[tokenID].latestBatchID < newLatestBatchID) {
                        result[tokenID] = Object.assign(result[tokenID], { latestBatchID: newLatestBatchID, latestIndex: coinlen - 1 });
                    }
                    console.log(`activeScanCoins: finish decrypt coins for batch #${bID}, token ${tokenID.slice(58)}`);
                })

                if (updateStorageSemaphore.count > 0) {
                    await updateStorageSemaphore.use(async () => {
                        const isLatest = bID === batchesToScan[batchesToScan.length - 1];
                        if (!isLatest || tokenID === PRVIDSTR) {
                            i++;
                            await this.setStorageCoinsScan(result);
                            if ((i % 5 === 0 || !started) && !cfg.finishScan) {
                ...
                return decryptedCoinList;
            } catch (e) {
                console.error(`scan coins batch ${bID} error:`, e);
                throw e;
            }  
        }))
    }
    console.log("[SDK][scanCoins] BB ")

    await Promise.all(tasks.flat());
    await Promise.all(tokenIDs.map(async tokenID => {
        Object.assign(result[tokenID], await this.groupSpentCoins([], tokenID, result[tokenID]));
    }));

    result.finishScan = true;
    await this.loopStorageCoins(result)

    console.log("[SDK][scanCoins] CC ", result)
