...
import { OTAKeyType, ReadonlyKeyType, PriKeyType } from "@lib/core";
import { wasm, callwasm } from "@lib/wasm";
import uniqBy from "lodash/uniqBy";
import { PTOKEN_ID } from "@lib/core";
import { parseCoinKeyImage, decryptCoinAmount } from "@lib/module/Account/features/HardwareWallet";
...
async function _scanCoins(cfg = {}, batchSize = ScanCoinBatchSize, poolSize = ScanCoinPoolSize) {
    ...
    const privkey = Boolean(this.key.KeySet.PrivateKey) ? this.key.base58CheckSerialize(PriKeyType)
    ...
    for (const tokenID of tokenIDs) {
            ...
            await this.rpc.getOutputCoinByIndex(batchStart, batchEnd, shardID, tokenID)
                .then(coinMap => {
                    ...
                    wqueues[wi] = wqueues[wi].then(async lst => {
                        ...
                        // if (!this.key.KeySet.PrivateKey) {
                        //     const prms = callwasm(wpool[wi], 'decryptCoinList',
                        //         JSON.stringify({
                        //             CoinList: decryptedCoinList,
                        //             KeySet: this.key.base58CheckSerialize(ReadonlyKeyType),
                        //             TokenList: tokenList,
                        //         }))
                        //     decryptedCoinList = JSON.parse(await prms);
                        //     if (!decryptedCoinList?.length) decryptedCoinList = [];
                        // }
        ...
        if (this.hwTransport) {
            for (let i = 0; i < resultForToken.length; i++) {
                await decryptCoinAmount(this.hwTransport, resultForToken[i]);
                await parseCoinKeyImage(this.hwTransport, resultForToken[i]);
            }
        }
