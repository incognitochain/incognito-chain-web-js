...
import { PriKeyType, PRVIDSTR } from "@lib/core";
...
async function activeScanCoins(cfg = {}) {
    ...
    for (const tokenID of tokenIDs) {
        if (!cfg[tokenID] || !cfg[tokenID].missingBatchIDs) cfg[tokenID] = { missingBatchIDs : {}};
        const coinlen = lengths[tokenID][shardID];
        const lastIndex = cfg[tokenID].latestIndex || 0;
        ...
        for (let i = 0; i < lst.length; i++) {
            ...
            try {
                ...
                if (bID === cfg[tokenID].latestBatchID && batchStart < lastIndex + 1 && !cfg[tokenID].missingBatchIDs[bID]) {
                ...
                console.log(`group coins index #${batchStart} - #${batchEnd}, shard ${shardID}, token ${tokenID.slice(58)}`, { coins: result });
