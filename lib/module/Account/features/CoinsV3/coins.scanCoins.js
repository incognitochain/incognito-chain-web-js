...
async function activeScanCoinsWithWebWorkers(cfg = {}) {
    ...
    if (!globalThis.Worker) throw 'Worker not found';
    const poolSize = Math.max(...coinGapsByToken) > ScanCoinSpawnWorkerThreshold ? ScanCoinPoolSize : 1;
    let wpool = Array(poolSize).fill(null)
    if (poolSize > 1) wpool = wpool.map(_ => (new Worker()));
    let wqueues = Array(poolSize).fill(Promise.resolve([]));
    let updateProgress = Promise.resolve(null);
    ...
    for (const tokenID of tokenIDs) {
        ...
        let mat = [];
        while (batchesToScan.length > 0) {
            mat.push(batchesToScan.splice(0, poolSize));
        }
        for (let i = 0; i < mat.length; i++) {
            const row = mat[i];            
            const tasks = row.map(async (bID, j) => {
                try {
                    let batchStart = bID * ScanCoinBatchSize;
                    let batchEnd = batchStart + ScanCoinBatchSize - 1;
                    ...
                    const prms = wqueues[j].then(_ => this.rpc.getOutputCoinByIndex(batchStart, batchEnd, shardID, tokenID)
                        .then(async coinMap => {
                            let decryptedCoinList = JSON.parse(await callwasm(wpool[j], 'decryptCoinList', JSON.stringify({
                                CoinList: Object.values(coinMap),
                                KeySet: privkey,
                                TokenList: tokenList,
                            }))) || [];
                            ...
                            delete result[tokenID].missingBatchIDs[bID];
                            console.log(`activeScanCoins: finish scan coins for index #${batchStart} - #${batchEnd}, shard ${shardID}, token ${tokenID.slice(58)}`);
            ...
            try {
                async function updateFn() {
                    const resultForToken = [].concat(...await Promise.all(tasks));
                    result[tokenID] = Object.assign(await this.groupSpentCoins(resultForToken, tokenID, result[tokenID]), { latestBatchID: newLatestBatchID, latestIndex: coinlen - 1 });
                    const isLatest = i === mat.length - 1;
                    if (!isLatest || tokenID === PRVIDSTR) {
                        await this.setStorageCoinsScan(result);
                        if (!cfg.finishScan) {
                            await this.sleep(150);
                            const num = row[row.length - 1] * ScanCoinBatchSize;
                            const storeStatus = { coinsLen, prvLen, tokenLen, tokenID, num };
                            await this.setAccountStorage(COINS_INDEX_STORAGE_KEY, storeStatus);
                            // started = true
                        }
                    }
                }
                updateProgress = updateProgress.then(updateFn.bind(this));
            } catch (e) {
                console.error(`store coins progress error:`, e);
                throw e;
            }
        }
    }
