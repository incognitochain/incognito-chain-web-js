...
function getNFTUnspentCoinsKey() {
    try {
        const fullNode = this.rpc.rpcHttpService.url;
        new Validator("getNFTUnspentCoinsKey-fullNode", fullNode)
            .required()
            .string();
        const otaKey = this.getOTAKey();
        return `${otaKey}${fullNode}${COINS_STORAGE_KEY.NFT_UNSPENT_COINS}`
    } catch (error) {
        throw error;
    }
}

// Get Un Spend Coins of NFT (maybe) list 
async function getNFTUnspentCoinsList() {
    const key = this.getNFTUnspentCoinsKey();
    const coinList = await this.getAccountStorage(key) || []
    return coinList;
}

// Get Un Spend Coins of NFT (maybe) list 
async function getNFTAssetTagsList() {
    const coinList = await this.getNFTUnspentCoinsList() || [];
    const assetTagList = coinList.map(coin => coin.AssetTag) || []
    return assetTagList;
}

// Set all coins scanned to storage
async function setNFTUnspentCoinsList(newNFTUNSpentList) {
    const key = this.getNFTUnspentCoinsKey();
    await this.setAccountStorage(key, newNFTUNSpentList)
}
...
export default {
    ...
    //NFT Unspent Coins
    getNFTUnspentCoinsKey,
    setNFTUnspentCoinsList,
    getNFTUnspentCoinsList,
    getNFTAssetTagsList
}