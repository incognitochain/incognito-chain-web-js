...
import { BurningPRVRequestMeta, PrivacyVersion, PRVIDSTR } from "@lib/core/constants";
...
async function createAndSendBurningPegPRVRequestTx({
  transfer: { fee, tokenID = PRVIDSTR, info = "", prvPayments = [], tokenPayments = [], paymentTokenID },
  extra: {
    burningType,
    burnAmount,
    remoteAddress,
  ...
  try {
    ...
    new Validator(
      ...
      burningType
    ).inList([BurningPRVERC20RequestMeta, BurningPRVBEP20RequestMeta, BurningPRVRequestMeta]);
    ...
    let otaReceiver = "";
    if (burningType == BurningPRVRequestMeta) {
