...
function mappingPTokenHistory({ history }) {
  try {
    ...
    const { createdAt, addressType } = h;
    const time = new Date(createdAt).getTime();
    const isShieldTx = addressType === ADDRESS_TYPE.SHIELD;
    const isUnshieldTx = addressType === ADDRESS_TYPE.UNSHIELD;
    h = {
      ...h,
      isShieldTx,
      isUnshieldTx,
...
function handleMappingTxsPTokenByTxsTransactorAndTxsReceiver({
  txsPToken,
  txsTransactor,
  txsReceiver,
}) {
  ...
  // mapping TxsPToken by TxsReceiver
  
  try {
    txsReceiver?.forEach((txt) => {
      let metaData = JSON.parse(txt?.metaData);
      if (metaData.Type === bridgeaggMeta.IssuingUnifiedRewardResponseMeta) {
        const requestedTxID = metaData?.RequestedTxID;
        const foundTxpIndex = _txsPToken.findIndex(
          (txp) =>
            txp.incognitoTx === requestedTxID ||
            txp.incognitoTxToPayOutsideChainFee === requestedTxID
        );
        if (foundTxpIndex > -1) {
          _txsPToken[foundTxpIndex].rewardAmount = txt.amount;
        }
      }
    });
  } catch (error) {
    console.log("MAPPING TXS PTOKEN BY TXS RECEIVER FAIL", error);
  }
...
export default {
  handleMappingTxsPTokenByTxsTransactorAndTxsReceiver,
