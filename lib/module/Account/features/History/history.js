    ...
    switch (version) {
      ...
      case PrivacyVersion.ver2: {
        try {
          ...
          // Combine Tx Status from BE Lam
          const txsUnshieldLocal = await this.getUnshiledEVMHistoryFromStorage()
          const { txsTransactorNew, txsTransactorMatchTxUnshieldLocal}= await this.handleFilterTxsTransactorByTxsUnshieldLocal(result.txsTransactor , txsUnshieldLocal)

          result.txsTransactor = txsTransactorNew || [];
          result.txsUnshield = txsTransactorMatchTxUnshieldLocal || [];

          console.log("========== Get history after mapping  Unshield Local ==========");
          console.log("txsTransactor: ", result.txsTransactor.length);
          console.log("txsReceiver: ", result.txsReceiver.length);
          console.log("txsPToken: ", result.txsPToken.length);
          console.log("txsPortal: ", result.txsPortal.length);
          console.log("txsUnshield: ", result.txsUnshield?.length);
