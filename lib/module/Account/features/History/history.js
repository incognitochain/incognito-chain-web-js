    ...
    switch (version) {
      ...
      case PrivacyVersion.ver2: {
        try {
          ...
          console.log("[SDK] [handleMeasureGetTxsHistory] result ", result);

          // Combine Tx Status from BE Lam
          const txsUnshield = await this.getUnshiledEVMHistoryFromStorage()
          result.txsUnshield = txsUnshield || [];
          
          // result = await this.fitlerUnShieldTxListFromStorage(result, txsUnshield)

          console.log("[SDK] [handleMeasureGetTxsHistory] txsUnshield Combine Local ", txsUnshield);
