...
function getKeyTxHistoryByTokenId(params) {
  try {
    ...
    new Validator("getKeyTxHistoryByTokenId-version", version)
      .required()
      .number();
    let keyByTokenId = this.getKeyStorageByTokenId(params);
    if (version === PrivacyVersion.ver3) {
      const fullNode = this.rpc.rpcHttpService.url;
      keyByTokenId = `${keyByTokenId}-${fullNode}`
    }
    ...
    if (tx) {
      try {
        ...
        if (version !== PrivacyVersion.ver3) {
          ...
          status = await this.rpc.getTransactionByHash(txId, true)
        }
...
async function handleMeasureGetTxsHistory(params) {
  ...
  console.log("[SDK] handleMeasureGetTxsHistory => ", params)
  ...
  try {
    ...
    switch (version) {
      ...
      case PrivacyVersion.ver3:
        try {
          console.log("[SDK] getTxsTransactorFromStorage => ", {
            tokenID,
            version,
          })
          let txsTransactor = await this.getTxsTransactorFromStorage({
            tokenID,
            version,
          });
          result.txsTransactor = txsTransactor || [];
