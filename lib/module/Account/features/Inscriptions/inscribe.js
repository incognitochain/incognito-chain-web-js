...
import { createCoin, estimateFee, estimateTxSize, prepareInputForTxV2 } from "@lib/module/Account/account.utils";
...
async function createAndSendInscribeRequestTx({
    ...
    try {
        ...
        for (const payment of prvPayments) {
            totalAmountBN = totalAmountBN.add(new bn(payment.Amount));
        }
        ...
        try {
            inputForTx = await prepareInputForTxV2({
                ...
                fee: new bn(INSC_MIN_FEE_PER_TX),
                tokenID,
                account: this,
                version: 2,
            });
