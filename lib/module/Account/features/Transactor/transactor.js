...
import { base64Encode, randBytes, stringToBytes } from "@lib/privacy/utils";
...
import { hwBeforeCreate, hwAfterCreate } from '@lib/module/Account/features/HardwareWallet';
...
async function transact({
  transfer: {
    ...
    tokenID = PRVIDSTR,
    tokenParams = null,
    tokenCoinChooser = defaultCoinChooser,
    tokenCoinForRingCount = NUMB_OF_OTHER_PKS,
  } = {},
  extra: {
    metadata = null,
    txHandler = null,
  ...
  let txParams = newParamTxV2(
    this.key,
    prvPayments,
    inputForTx.inputCoinStrs.map(c => Object.assign({}, c, { SharedRandom: '' })),
    fee,
    null,
    metadata,
    info,
  ...
  if (!!tokenPayments) {
    ...
    if (isTransfer) {
      try {
        inputForToken = await prepareInputForTxV2({
          ...
          account: this,
          version,
          coinChooser: tokenCoinChooser,
          numOfOtherPks: tokenCoinForRingCount,
        });
      } catch (e) {
        throw new CustomError(
          ErrorObject.InitNormalTxErr,
    ...
    tokenParams = newTokenParamV2(
      tokenPayments,
      inputForToken.inputCoinStrs.map(c => Object.assign({}, c, { SharedRandom: '' })),
      tokenID,
  ...
  if (this.hwTransport) {
    const cseed = await hwBeforeCreate(this.hwTransport,
      (tokenPayments?.length > 0) ? tokenPayments : prvPayments,
      inputForTx.inputCoinStrs, inputForToken.inputCoinStrs, firstC);
    const temp = await randBytes(cseed.length);
    const piParam = temp.map(b => b % 8);
    txParams.Kvargs = { cseed: cseed, pi: piParam };
  }
  ...
  if (this.hwTransport) {
    txObj = await hwAfterCreate(this.hwTransport, txObj, inputForTx.inputCoinStrs, txParams.Kvargs.pi, inputForToken?.inputCoinStrs);
    b58EncodedTx = checkEncode(Buffer.from(JSON.stringify(txObj)));
  }
  ...
  if (typeof txHandler === "function") {
    ...
    try {
      if (version === PrivacyVersion.ver3) {
        if (tokenID === PRVIDSTR) {
          response = await this.rpc.sendRawTx(b58EncodedTx)
        } else {
          response = await this.rpc.sendRawTxCustomTokenPrivacy(b58EncodedTx)
        }
      } else {
