...
import {KeyWallet, OTAKeyType, PaymentAddressType, PriKeyType, ReadonlyKeyType,} from "@lib/core";
...
async function getOTAReceiveWithCfg({ senderShardID } = {}) {
  try {
    console.log("getOTAReceiveWithCfg senderShardID ", senderShardID)
    new Validator("checkStatusListUnspentCoinsStorage-senderShardID", senderShardID)
        .required()
        .number();
    return wasm.createOTAReceiverWithCfg(JSON.stringify({
    ...
    console.log("getOTAReceiveWithCfg error ", error)
    throw new CustomError(
        ErrorObject.WasmOTAReceiverError,
        ErrorObject.WasmOTAReceiverError.description
    );
