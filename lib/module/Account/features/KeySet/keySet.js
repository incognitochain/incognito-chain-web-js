...
function getPrivateKey() {
  if (this.key.KeySet.PrivateKey) return null;
  return this.key.base58CheckSerialize(PriKeyType);
}

function getReadonlyKey() {
  if (this.key.KeySet.ReadonlyKey) return null;
...
async function getDeserializeInformationNoCache() {
  const privateKey = this.getPrivateKey();
  const miningSeedKey = hashSha3BytesToBytes(
    hashSha3BytesToBytes(this.key.KeySet.PrivateKey)
  );
...
async function getOTAReceiveWithCfg({ senderShardID } = {}) {
  ...
  try {
    otaReceiverWithCfg = wasm.createOTAReceiverWithCfg(JSON.stringify({
...
export default {
  ...
  getDeserializeInformationNoCache,
