...
import { getBurningAddress, PDEX_ACCESS_ID, PRVIDSTR } from "@lib/core";
...
class Account {
  ...
  async getFollowTokensBalance({ defaultTokens = [], version = PrivacyVersion.ver2 }) {
    try {
      ...
      followTokens = followTokens.filter(tokenID => tokenID !== PDEX_ACCESS_ID);
